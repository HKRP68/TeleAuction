...
def rtm_check_text(player_row, eligible: list) -> str:
    """Sent to Team A after timer expires — USE RTM or PASS buttons. Plain text only."""
    aid    = player_row["auction_id"]
    team_a = ", ".join(team_display(r) for r in eligible)
    return (
        f"🔄 RTM CHALLENGE!\n"
        f"{'═'*20}\n\n"
        f"🏏 {flag(player_row['nationality'])} {player_row['name']}\n"
        f"💰 Winning Bid: {fmt(live.current_bid, aid)} by {live.highest_bidder_name}\n\n"
...
def rtm_bid_raised_text(player_row, new_bid: int) -> str:
    ...
    return (
        ...
        f"{'═'*20}\n\n"
        f"{live.rtm_orig_bidder_name} raised to {fmt(new_bid, aid)}!\n\n"
        f"🏏 {player_row['name']}\n"
        f"📈 Increase: +{fmt(diff, aid)}\n\n"
        f"{live.rtm_team_name}, will you match this final amount?\n\n"
        f"✅ MATCH → Player sold to you for {fmt(new_bid, aid)}\n"
        f"❌ DECLINE → Player goes to {live.rtm_orig_bidder_name} for {fmt(new_bid, aid)}\n\n"
...
def rtm_no_raise_text(player_row, orig_bid: int, rtm_team: str,
    ...
    return (
        ...
        f"💰 Final Price: {_cr(orig_bid)} Cr (Original amount)\n"
        f"🏆 Winner: {rtm_team} 🎴 (via RTM)\n\n"
        f"📊 Transaction:\n"
        f"• Deducted: {_cr(orig_bid)} Cr from {rtm_team}\n"
...
def rtm_raise_error_text(original_team: str, your_team: str) -> str:
    return (
        f"❌ *RAISE BID ERROR*\n"
        f"{'═'*20}\n\n"
        f"Only {md_safe(original_team)} (current highest bidder) can raise the bid!\n\n"
        f"You are: {your_team}\n\n"
...
async def _mark_unsold(context: ContextTypes.DEFAULT_TYPE, pr):
    ...
    msg = await context.bot.send_message(
        chat_id=live.chat_id,
        text=f"❌ {md_safe(pr['name'])} goes *UNSOLD!* No bids received.",
        parse_mode=ParseMode.MARKDOWN,
...
async def process_bid(update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if live.rtm_state == RTM_ACTIVE and uid == live.rtm_orig_bidder_id:
        ...
        if update.callback_query:
            ...
            await update.message.reply_text(
                ...
                f"waiting for {md_safe(live.rtm_team_name)} to decide!",
                parse_mode=ParseMode.MARKDOWN,
            )
        return
    ...
    new_msg = await context.bot.send_message(
        ...
        text=(
            f"💥 *New Bid*\n{'─'*28}\n"
            f"Player: {md_safe(pr['name'])}\n"
            f"Amount: *{fmt(bid_l,aid)}*\n"
            f"By: {md_safe(bid_display(part))}\n"
            f"{outbid}\n"
            f"⏱ Timer: {duration}s"
        ),
        parse_mode=ParseMode.MARKDOWN,
...
async def cmd_squad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sq:
        await update.message.reply_text(f"{md_safe(team_display(row))} — Squad empty.",
                                        parse_mode=ParseMode.MARKDOWN)
        return
    ...
    lines = [f"🏏 {md_safe(team_display(row))} ({len(sq)} players)\n{'─'*28}"]
...
async def cmd_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, r in enumerate(parts, 1):
        ...
        lines.append(f"{m} {md_safe(team_display(r))}\n  Spent: {fmt(r['total_spent'],aid)} | {sq} players")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN)
...
async def cmd_teamup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🤝 *TeamUp!* {db.display(proxy)} can now bid as {md_safe(team_display(p1))}.",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cmd_add_to_squad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = db.get_part(aid, uid)
    await update.message.reply_text(
        f"Added {len(names)} player(s) to {md_safe(team_display(row))}",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cmd_remove_from_squad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Removed positions {positions} from {md_safe(team_display(row))}",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cmd_clear_squad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.cx.execute("UPDATE participants SET squad='[]',total_spent=0 WHERE auction_id=? AND user_id=?",
                  (live.auction_id, uid))
    db.cx.commit()
    await update.message.reply_text(f"Squad cleared for {md_safe(team_display(row))}",
                                    parse_mode=ParseMode.MARKDOWN)
...
async def cmd_swap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Swapped purse & squad: {md_safe(team_display(r1))} ↔ {md_safe(team_display(r2))}",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cmd_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_live_state()
    msg = await update.message.reply_text(
        f"⏭ {md_safe(pr['name'])} passed (UNSOLD).",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=reauction_keyboard(),
    )
    live.reauction_msg_id = msg.message_id
...
async def _handle_rtm_use(update, context: ContextTypes.DEFAULT_TYPE, uid: int):
    ...
    if rtm_team_assigned.lower() != ipl_team.strip().lower():
        await err(
            f"Your RTM card is for {md_safe(rtm_team_assigned or 'N/A')}, "
    ...
    msg = await context.bot.send_message(
        chat_id=live.chat_id,
        text=(
            f"🎴 {md_safe(live.rtm_team_name)} uses RTM on {md_safe(pr['name'])}!\n"
            f"{'─'*28}\n"
            f"Current highest bid: *{fmt(live.rtm_orig_bid, aid)}* "
            f"by {md_safe(live.rtm_orig_bidder_name)}\n\n"
            f"{md_safe(live.rtm_orig_bidder_name)} — raise your bid using:\n"
            f"`/bid <amount>`  e.g. `/bid 19cr`\n\n"
            f"⏱ You have *{Config.RTM_TIMER}s* to counter. "
            f"If no counter, {md_safe(live.rtm_team_name)} wins the player at "
            f"*{fmt(live.rtm_orig_bid, aid)}*."
        ),
        parse_mode=ParseMode.MARKDOWN,
    )
